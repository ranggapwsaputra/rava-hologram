#!/usr/bin/env python3
"""
R.A.V.A Desktop Voice Agent — Jarvis-style welcome + WebSocket TTS bridge.

Features:
  1. Double-clap detection: reads the default microphone and triggers a
     welcome flow (Spotify, Chrome windows, ElevenLabs voice, Cursor) when
     two loud transients are detected within a short time window.
  2. WebSocket TTS bridge: listens on ws://localhost:JARVIS_WS_PORT (default 7788)
     so the React frontend (GlobalVoiceAgent) can send speak commands and receive
     status updates, routing all speech through ElevenLabs instead of the browser.

Run:
  python -m pip install -r requirements.txt
  python jarvis.py

WebSocket protocol (frontend → server):
  {"action": "speak", "text": "Hello, Sir."}       — play TTS
  {"action": "cancel"}                              — stop current playback (future)

WebSocket protocol (server → frontend):
  {"status": "ready"}                               — connection established
  {"status": "speaking", "text": "..."}             — playback started
...
# Spotify / Audio on clap (empty = disabled)
SONG_URI = ""

# Cursor settings (disabled on double clap)
...
# ─── WebSocket TTS bridge ────────────────────────────────────────────────────
# Port for the local WebSocket server that the React frontend connects to.
# Override via JARVIS_WS_PORT environment variable.
JARVIS_WS_HOST = "127.0.0.1"
JARVIS_WS_PORT = 7788
# Lock so only one TTS utterance plays at a time (clap + WebSocket may overlap).
_tts_lock = threading.Lock()

# Load .env first (primary), then .env.local as override (for local dev)
...
# ─── ElevenLabs on-demand TTS (used by WebSocket bridge) ────────────────────

def say_elevenlabs_text(text: str) -> None:
    """Speak arbitrary text via ElevenLabs (thread-safe via _tts_lock)."""
    if not text.strip():
        return
    vid, model_id, output_format, pcm_rate = elevenlabs_env_config()
    if not vid:
        log.warning("[WS] ELEVENLABS_VOICE_ID not set — cannot speak via bridge.")
        return
    api_key = (os.environ.get("ELEVENLABS_API_KEY") or "").strip()
    if not api_key:
        log.warning("[WS] ELEVENLABS_API_KEY not set — cannot speak via bridge.")
        return
    try:
        from elevenlabs.client import ElevenLabs
    except ImportError:
        log.warning("[WS] Install dependencies: pip install -r requirements.txt")
        return
    with _tts_lock:
        try:
            client = ElevenLabs(api_key=api_key)
            ...
            log.warning("[WS] ElevenLabs TTS failed: %s", e)
            return
        if not raw:
            log.warning("[WS] ElevenLabs returned empty audio.")
            return
        pcm_i16 = np.frombuffer(raw, dtype=np.int16)
        pcm_f = pcm_i16.astype(np.float32) / 32768.0
        try:
            ...
            log.warning("[WS] Could not play TTS audio: %s", e)
...
async def _ws_handler(websocket: any) -> None:  # type: ignore[type-arg]
    """Handle one WebSocket client connection from the React frontend."""
    client = websocket.remote_address if hasattr(websocket, "remote_address") else "?"
    ...
    try:
        ...
        async for raw in websocket:
            ...
            if action == "speak":
                ...
                if not text:
                    ...
                    continue
                log.info("[WS] speak → %r", text[:80])
                await websocket.send(json.dumps({"status": "speaking", "text": text}))
                # Run blocking TTS in an executor so we don't block the event loop
                loop = asyncio.get_running_loop()
                await loop.run_in_executor(None, say_elevenlabs_text, text)
                await websocket.send(json.dumps({"status": "done"}))

            elif action == "cancel":
                # Future: stop ongoing playback
                sd.stop()
                await websocket.send(json.dumps({"status": "done"}))
        ...
        log.info("[WS] Client %s disconnected: %s", client, e)
    finally:
        _connected_clients.discard(websocket)


async def _run_ws_server() -> None:
    """Start the WebSocket TTS bridge server."""
    try:
        import websockets  # noqa: F401
    except ImportError:
        log.error("[WS] 'websockets' not installed. Run: pip install -r requirements.txt")
        return

    import websockets.server  # type: ignore

    port = int((os.environ.get("JARVIS_WS_PORT") or "").strip() or JARVIS_WS_PORT)
    log.info("[WS] WebSocket TTS bridge listening on ws://%s:%d", JARVIS_WS_HOST, port)
    async with websockets.server.serve(_ws_handler, JARVIS_WS_HOST, port):
        await asyncio.Future()  # run forever


# ─── Clap detection loop (runs in a background thread) ──────────────────────

def _clap_loop(blocksize: int) -> None:
    """Blocking mic loop — run in a daemon thread so asyncio can own the main thread."""
    noise_floor = 1e-4
    ...
    log.info(
        "[Clap] Listening (double clap: %.2f–%.2fs apart, rate=%d, block=%d ms, "
    ...
    try:
        with sd.InputStream(
            ...
            while True:
                ...
                if overflowed:
                    log.warning("[Clap] Input overflow; try a larger BLOCK_MS")
                ...
                if (
                    ...
                    if first_clap_time is None:
                        ...
                        if gap < MIN_DOUBLE_GAP_S:
                            ...
                            log.info(
                                "[Clap] Double clap detected (gap=%.3fs, rms=%.5f, "
        ...
        log.info("[Clap] Stopped by KeyboardInterrupt.")
    except sd.PortAudioError as e:
        log.error("[Clap] Audio error: %s", e)
        log.error("[Clap] If PortAudio fails, install/repair drivers or try another SAMPLE_RATE.")
...
def _log_startup_info() -> None:
    if SONG_URI.strip():
        ...
        log.info("SONG_URI is empty — no song on double clap.")
    if FOCUS_EXISTING_CURSOR_ON_DOUBLE_CLAP:
        log.info(
    ...
    if OPEN_CLAUDE_CODE_IN_CHROME:
        ...
        log.info(
            "After double clap, open Claude in Chrome%s on monitor %d: %s",
    ...
    if OPEN_BINANCE_BTC_IN_CHROME:
        ...
        log.info(
            "After double clap, open Binance BTC in Chrome%s on monitor %d: %s",
            " fullscreen" if OPEN_CHROME_FULLSCREEN else "",
...
async def _async_main() -> None:
    ...
    clap_thread = threading.Thread(target=_clap_loop, args=(blocksize,), daemon=True)
    clap_thread.start()

    # Run WebSocket server on the asyncio event loop (blocks until Ctrl+C)
...
def main() -> int:
    try:
        ...
        log.info("R.A.V.A Voice Agent stopped.")
    return 0
